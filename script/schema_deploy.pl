...
my ( $opt, $usage ) = describe_options(
    "%c %o",
    [ 'db-host',        "Host of PG (DB_HOST or localhost by default)", { default => $ENV{DB_HOST} || 'localhost' } ],
    [ 'db-port',        "Port of PG (5432 by default)",      { default => 5432 } ],
    [ 'db-user',        "User to connect to PG (DB_USER)",   { default => $ENV{DB_USER} } ],
    [ 'db-password',    "Password for given user (DB_PASS)", { default => $ENV{DB_PASS} } ],
...
my $schema = X::Schema->connect(
    sprintf( 'dbi:Pg:dbname=x;host=%s;port=%d', $opt->db_host, $opt->db_port ),
    $opt->db_user, $opt->db_password, {AutoCommit => 1, RaiseError => 1}
);
